...
// 	this.scene.add( ground );
    ...
    function getIntersectsTopDown(point, object, recursive, color) {
        ...
        // var distance = 1000; 
        // at what distance to determine pointB
        // var _distance = -1000;
        
        // var _direction = new THREE.Vector3(0,0,-1);
        // var pointB = new THREE.Vector3();
        // pointB.addVectors ( pointA, _direction.multiplyScalar( _distance ) );
	...
	transformControls.addEventListener('change', function() {
		...
		if (object !== undefined) {
			if ( object.name == "EdgeHelper" ) {
				...
				var intersected = getIntersectsTopDown(worldObjectPosition, parcelToscan, true, 0xFFFF00);
				...
				if(intersected.length > 0 && overParcel) {
                    ...
                    case 'parcel':
                        ...
                        if (intersects.length > 0) {
                            ...
                            if (scope.lastSelectedParcel.names.length && scope.lastSelectedParcel.names.indexOf(parcel.userData.name) >= 0) {
                                ...
                                var index = scope.lastSelectedParcel.names.indexOf(parcel.userData.name);
                                var parcel = scope.lastSelectedParcel.parcels[index];
                                
                                //if it's not edited we remove it from the scope
                                if(parcel.getObjectByName("EdgeHelpers") == undefined) {
                        ...
                        // 			build.visible = true;
                            ...
                            //if parcel is not in the editing scope
                            // if (scope.lastSelectedParcel.names.length && scope.lastSelectedParcel.names.indexOf(parcel.userData.name) == -1) {
                            ...
                            //         console.log("child", scope.lastSelectedParcel.parcels)
                            //     scope.lastSelectedParcel.parcels.forEach(function(child){
                            //         child.material = new THREE.MeshPhongMaterial({color: colors.parcel, opacity: 0.1, transparent: true});
                            //         var footprint = child.getObjectByName('footprint');
                            //         if( footprint !== undefined ) footprint.visible = false;
                            //     });
                                
                                
                            //     // scope.lastSelectedParcel.parcel.visible = false;
                            //     // for (var i = 0; i < scope.lastSelectedParcel.parcel.children.length; i++) {
                            //     //     Things[i]
                            //     // }
                            ...
                            if( parcel.userData.footprint == undefined ) {
                                ...
                                var footprintShape = THREE.ParcelJsonLoader.createSlicedShape(parcel.userData.shape, slicePlane);
                        ...
                        // 	this.inputSurface.innerHTML = Math.floor( parcel.userData.surface * 100 ) / 100;
                            ...
                            // console.log('parcelCenter: ', parcelCenter);
...
// 			}
    ...
    this.buildingsGround = new THREE.Object3D();
    this.buildingsGround.name = "buildingsGround";
    this.buildingsGround.position.set(0, 0, this.R * 1000);
    
    this.buildingsLatRot = new THREE.Object3D();
    this.buildingsLatRot.add(this.buildingsGround);
    
    this.buidlingsLonRot = new THREE.Object3D();
    this.buidlingsLonRot.add(this.buildingsLatRot);
     
    this.buildings = new THREE.Object3D();
    this.buildings.name = "Buildings group";
    
    this.buildings.add(this.buidlingsLonRot);
    ...
    this.updateScene = function(position) {
        ...
        if (this.zoom >= World.ZOOM_FLAT) {
            ...
            _this2.buildingsLatRot.rotation.set(-latOri * Math.PI / 180, 0, 0);
            _this2.buildingsLatRot.rotation.set(0, lonOri * Math.PI / 180, 0);
                ...
                var _loop3 = function _loop3(btile) {
                    ...
                    if (!tiles.hasOwnProperty(id)) {
                        ...
                        (function () {
                            ...
                            buildingSupport = new THREE.Object3D();
                                    ...
                                    switch (_this2.terrains[terrainId].type) {
                                        case 'terrain':
                                            ...
                                            (function (tileSupport, tileMesh, zoom, xtile, ytile) {
                                                ...
                                                _this2.buildingsGround.add(buildingSupport);
                            ...
                            var _loop4 = function _loop4(layerId) {
                                ...
                                switch (_this2.layers[layerId].type) {
                                    case 'tile':
                                        ...
                                        break;
                                    // case 'building':
                                    //         console.log('building zoom', _this2.zoom );
                                    //     if (_this2.zoom >= 14 && zoom_ >= Math.max(_this2.zoom , _this2.layers[layerId].minZoom)) {
                                    ...
                                    //             if(_this2.loaders[layerId].loadedTiles.indexOf('tile:'+zoom+':'+xtile+':'+ytile) >= 0) return;
                                    ...
                                    //                 _this2.loaders[layerId].loadedTiles.push('tile:'+zoom+':'+xtile+':'+ytile);
                                    //                 // console.log(buildingSupport);
                                    ...
                                    //                     // console.log(building.userData);
                                    //                     var buildingId = building.userData.osm.id;
                                                        
                                    //                     if( _this2.buildingIds.indexOf(buildingId) == -1 ) {
                                                            
                                    //                         parcelSupport.add(building);
                                    ...
                                    case 'building':
                                        ...
                                        break;
                                    case 'parcel':
                                        if (_this2.zoom >= 17 && zoom_ >= Math.max(_this2.zoom, _this2.layers[layerId].minZoom)) {
