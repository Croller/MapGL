...
window.Slicer = require('./slice.js')(window.THREE);
...
module.exports = function(THREE) {
    ...
    Slicer.sliceGeometry = function(geometry, plane, closeHoles) {
    ...
    Slicer.sliceShape = function(shape, plane, closeHoles) {
        var slicedPoints = [];
        var shapePoints = shape.extractPoints().shape;
        shapePoints = shapePoints.map(x => x = new THREE.Vector3(x.x,x.y,0));
        
        var builder = new ShapeBuilder(shapePoints, slicedPoints, plane);

        var distances = [];
        var positions = [];

        
        shapePoints.forEach(function(vec, key) {
            var distance = findDistance(vec, plane);
            var position = distanceAsPosition(distance);
            distances.push(distance);
            positions.push(position);
            ...
            if(lastKey < 0) return;
            // var lastKey = FACE_KEYS[FACE_KEYS.length - 1];
            var lastDistance = distances[lastKey];
            var lastPosition = positions[lastKey];
            if (position === FRONT) {
                if (lastPosition === BACK) {
                    builder.addIntersection(lastKey, key, lastDistance, distance);
                    builder.addVertex(key);
        ...
        var targetPoints = builder.targetShape.map(v => v = new THREE.Vector2(v.x,v.y));
        var sliced = new THREE.Shape(targetPoints);
    ...
    var ShapeBuilder = function(sourceShape, targetShape, slicePlane) {
    ...
    ShapeBuilder.prototype.addVertex = function(index) {
        //this.addUv(key);
        //this.addNormal(key);
        ...
        if (this.addedVertices.hasOwnProperty(index)) {
            ...
            var vertex = this.sourceShape[index];
            this.targetShape.push(vertex);
            newIndex = this.targetShape.length - 1;
            this.addedVertices[index] = newIndex;
        }
        //this.faceIndices.push(newIndex);
    };

    ShapeBuilder.prototype.addIntersection = function(indexA, indexB, distanceA, distanceB) {
       ...
       // this.addIntersectionNormal(keyA, keyB, t);
        ...
        if (this.addedIntersections.hasOwnProperty(id)) {
            ...
            var vertexA = this.sourceShape[indexA];
            var vertexB = this.sourceShape[indexB];
            var newVertex = vertexA.clone().lerp(vertexB, t);
            this.targetShape.push(newVertex);
            index = this.targetShape.length - 1;
    ...
    ShapeBuilder.prototype.intersectionId = function(indexA, indexB) {
